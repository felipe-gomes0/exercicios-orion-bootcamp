...
A pasta `sql-scripts` contém todos os *scripts* SQL necessários para a inicialização e inserção de dados no banco de dados.
...
* **Ordem de Execução e Conteúdo:**
    * `01-create-table-cursos.sql`: Cria a tabela `cursos`.
    * `02-create-table-alunos.sql`: Cria a tabela `alunos`, incluindo uma *Foreign Key* (`fk_alunos_cursos`) que referencia a tabela `cursos`.
    * `03-insert-cursos.sql`: Insere dados na tabela `cursos`.
    * `04-insert-alunos.sql`: Insere dados na tabela `alunos`.
